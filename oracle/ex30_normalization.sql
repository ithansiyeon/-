...
정규화, Normalization
- 자료의 손실이나 불필요한 정보를 없애고, 데이터의 일관성을 유지하며 데이터 종속을 최소화기 위해서 자료 구조(ERD)를 수정하는 작업
- 기존의 데이터베이스 구조를 (좀 더 나은 구조로) 개선하는 방법 
- 기존의 만든 테이블(비정형, 비정규화 상태) -> 정규화 -> 정규화 구조 
- 제 1 정규화 -> 제 2정규화 -> 제 3정규화 등..
- 제 1~3 정규화 + 역정규화 
...
함수 종속(Functional Dependency)
    ...
    - 정규화는 '부분 함수 종속이나 이행 함수 종속을 모두 없애고, 모든 컬럼의 관계를 완전 함수 종속'으로 만드는 작업 
...
역정규화
- 정규화된 결과를 다시 원래대로 되돌리는 작업 => 반정규화blu
- 2개 테이블 -> 1개 테이블 
- 수업 중 사용 X

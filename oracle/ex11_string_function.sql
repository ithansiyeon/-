...
select 
    *
from tblInsa
order by substr(ssn,3,2) asc; --태어난 월, 부분문자열 가지고 정렬 가능
...
select replace('홍길동','홍','김'),  replace('홍길동','이','김') from dual; --찾으면 바꾸지만 못 찾으면 적용될 대상이 없어서 원본 그대로 반환됨 
...
select 
    ...
    replace(replace(substr(ssn,8,1), '1','남자'),'2','여자') --replace(A,'2','여자')
from tblInsa;
...
-- ** 못찾으면 null 반환 -> count()(집계 함수) 자주 사용 => null을 제외한다. 집계 함수 
...
select 
    ...
    replace(replace(substr(ssn,8,1), '1','남자'),'2','여자'), --replace(A,'2','여자')
    decode(substr(ssn,8,1),'1','남자','2','여자') --case보다는 코딩량이 적고 replace 보다는 가독성이 좋음
from tblInsa;
...
select 
    ...
    decode(continent, 'AS','아시아','SA','아메리카','EU','유럽','AF','아프리카','AU','호주') as c3
from tblCountry;
...
from tblAddressBook; -- 강동구가 어디서 시작할지 몰라서 7이 아닐수도 있어서 0을 1로 바꿔서 

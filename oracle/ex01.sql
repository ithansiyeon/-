...
관계형 데이터베이스 관리시스템의 종류(RDBMS) => 표로 관리
1. Oracle
    - Oracle
    - 기업용 
2. MS-SQL
...
SQL, Structured Query Lanauage (query가 중요)
- SQL == Query
- 질의를 목적으로 만들어진 언어(질문을 하는 위주로 만든 언어)
- 자바에 비해 자연어에 가깝다.

1. DBMS와 SQL은 서로 독립적이다.
    - 표준 SQL(ANSI-SQL) 
    - 표준 SQL은 모든 DBMS에서 공통으로 사용한다.
    - SQL-86(년도) > SQL89 > ...SQL99 > SQL2011
2. 대화식 언어이다.
    - 질문 > 답변 > 질문 > 답변 > x 반복 (흐름이 존재하지 않아, 단일문장으로 이루어져 있어)
...
SELECT * FROM tabs; -- 오라클은 키워드는 대문자로 사용자 식별자는 소문자로 

-- 생성 -> 식별자 > 규칙(최대 30바이트 이하) 
create table tblTestaaaaaaaaaaaaaaaaaaaaaaa( - identifier is too long
    num number;
); 
-- table 만들때 쓰는 문법

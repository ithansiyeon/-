...
public class Index extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int maxDate = c.getActualMaximum(Calendar.DATE); // 해당 객체가 가지고 있는 max값을 반환함, 일자가 가질수 있는 가장 큰 숫자 , 31
		...
		for(int i=1;i<=maxDate;i++) {
			...
			for(PlanDTO dto : list) {
				//dto.getRegdate() -> 2020-08-20
				//달력 날짜 -> 
				
				if (dto.getRegdate().equals(String.format("%tF",c))){
					
					txtCal += String.format("<div class = 'item' data-toggle='modal' data-target='#myModal' onclick='setData(\"%s\",\"%s\")'>- %s</div>",dto.getRegdate(),dto.getName(),dto.getContent());
				}
		...
		now.set(year,month,1); // 지금 보고 있는 날짜 기준

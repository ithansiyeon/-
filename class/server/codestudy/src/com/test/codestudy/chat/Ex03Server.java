...
public class Ex03Server {
	...
	//현재 서버에 접속한 클라이언트의 WebSocket Session 목록(공유를 해야 돼서 static)
		...
		for(Session session : list) {
			if(session != userSession) {
				...
				try {
					...
					System.out.println("Ex03Server.handleOpen()");
					e.printStackTrace();
				}
			}
		}
		...
		for(Session session : list) {
			if(session!= userSession) {
				try {
					...
					System.out.println("Ex03Server.handleMessage()");
					e.printStackTrace();
				}
			}
		}

...
<body>
	<div class="container">
		...
		<div>
			Session ID :<%=session.getId()%></div>
		<div>
			Session Max Inactive Interval(세션 만료 시간) :	<%=session.getMaxInactiveInterval()%></div>
		...
		<div>
			color :
			<%=session.getAttribute("color")%></div>
		<div>
			data :
			<%=session.getAttribute("data")%></div>
		<%
			} else {
		%>
		<div>데이터 없음</div>
		...
		<div>
			<a href="ex17_reset.jsp">세션 초기화</a>
		</div>
		<div>
			<a href="ex17_interval.jsp">세션 만료시간 지정</a>
		</div>
